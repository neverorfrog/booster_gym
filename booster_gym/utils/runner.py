...
import signal
import imageio
# Import isaacgym-dependent modules before torch
from booster_gym.envs import *
import torch
import torch.nn.functional as F
from booster_gym.utils.model import *
from booster_gym.utils.buffer import ExperienceBuffer
from booster_gym.utils.utils import discount_values, surrogate_loss
from booster_gym.utils.recorder import Recorder
...
class Runner:
    ...
    def _update_cfg_from_args(self):
        cfg_file = os.path.join("booster_gym/envs", "{}.yaml".format(self.args.task))
    ...
    def _load(self):
        ...
        if (self.cfg["basic"]["checkpoint"] == "-1") or (self.cfg["basic"]["checkpoint"] == -1):
            checkpoint_files = sorted(glob.glob(os.path.join("logs", "**/*.pth"), recursive=True), key=os.path.getmtime)
            if not checkpoint_files:
                print("No checkpoint files found in logs directory. Starting with random weights.")
                self.cfg["basic"]["checkpoint"] = None
                return
            self.cfg["basic"]["checkpoint"] = checkpoint_files[-1]
